          ...
          <div className="flex flex-col -space-y-[1.5rem]">
            <AnimatePresence initial={false}>
              {tasks.map((log: TaskLog) => {
                ...
                return (
                  <motion.button key={log.id} layout initial={{ opacity: 0, y: -50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: 200 }} onClick={() => actions.handleUnstack(log.id)} className="relative group cursor-pointer w-full">
